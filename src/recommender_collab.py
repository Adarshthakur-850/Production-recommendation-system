...
class CollaborativeRecommender:
    ...
    def fit(self, df):
        ...
        self.user_item_matrix = df.pivot(index='user_id', columns='product_id', values='rating').fillna(0)
        
        # SVD
        self.model = TruncatedSVD(n_components=min(self.n_components, self.user_item_matrix.shape[1]-1), random_state=42)
    ...
    def predict_rating(self, user_id, product_id):
        if user_id not in self.user_item_matrix.index or product_id not in self.user_item_matrix.columns:
        ...
        pred = np.dot(self.matrix_reduced[user_idx], self.model.components_[:, prod_idx])
        return pred
